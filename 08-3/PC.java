...
class PC
{
	public static void main(String[] args)
	{
		...
		MB.Run();
		MB.BootPCI(null);              //通过主板MB启动插槽PCI，同时槽上没有建立任何设备对象（null）
		MB.BootPCI(new VisionCard());  //通过主板MB启动插槽PCI，同时槽上建立了显卡对象（PCI M = new VisionCard）
		MB.BootPCI(new SoundCard());   //通过主板MB启动插槽PCI，同时槽上建立了声卡对象（PCI M = new SoundCard）
	}
}